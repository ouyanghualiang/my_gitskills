...
PCRE_PATH:= $(GLIB_SOURCES_PATH)/glib/pcre
LOCAL_PATH:= $(GLIB_SOURCES_PATH)/glib

include $(CLEAR_VARS)

include $(LOCAL_PATH)/Sources.mk
include $(PCRE_PATH)/Sources.mk
...
LOCAL_MODULE:= glib
LOCAL_SRC_FILES:= $(GLIB_CORE_SOURCES) $(GNULIB_SOURCES) $(CHARSET_SOURCES) $(filter %.c, $(libpcre_la_SOURCES))
LOCAL_CFLAGS += -DGLIB_COMPILATION=1 -DNVALGRIND=1 $(libpcre_la_CPPFLAGS)
LOCAL_EXPORT_LDLIBS := -lz -lc -lm -lstdc++ -ldl
