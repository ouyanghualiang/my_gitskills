...
LOCAL_SRC_FILES := cairo/util/cairo-gobject/cairo-gobject-enums.c \
                   ...
                   $(addprefix cairo/src/, $(cairo_sources) $(cairo_gl_sources) $(cairo_egl_sources) \
                                           $(cairo_ft_sources))
LOCAL_C_INCLUDES := $(GLIB_SOURCES_PATH)/../include $(GLIB_SOURCES_PATH) \
                    $(GLIB_SOURCES_PATH)/gmodule $(GLIB_SOURCES_PATH)/glib $(GLIB_SOURCES_PATH)/glib/pcre \
                    $(LOCAL_PATH)/glib/glib \
                    $(MAKEFILE_PATH)/../fontconfig/distsrc $(MAKEFILE_PATH)/../freetype/distsrc/include \
                    $(NDK_ROOT)/sources/android/support/include \
                    $(NDK_ROOT)/sources/android/support/src/musl-locale
LOCAL_SHARED_LIBRARIES := glib fontconfig freetype gobject
